...
export const LegalConsultationSection = () => {
  return (
    <div className="relative flex w-full flex-col items-start gap-10 overflow-hidden px-6 py-12 sm:px-8 md:px-12 lg:px-24 lg:py-[60px] xl:min-h-[536px] xl:flex-row xl:items-center justify-between xl:gap-24 2xl:gap-[203px] xl:px-40">
      ...
      <div className="flex w-full flex-col items-center  gap-10 px-0 py-5 xl:flex-1">
        {cards.map((card, index) => (
          <div
            key={index}
            className="flex max-w-[454px] items-center gap-10"
            data-reveal="fade-up"
            style={{ ["--reveal-delay" as string]: `${index * 120}ms` }}
          >
            <div className="items-center pl-[30px] pr-4 py-4 flex-1 grow border-l-4 [border-left-style:solid] border-[#B99578] flex gap-4 relative rounded-md">
              <img
              ...
              <div className="flex flex-col items-start justify-center gap-2 relative flex-1 grow">
                ...
                <p className="[font-family:'Gotham-Book',Helvetica] font-normal text-[#444444] text-base relative self-stretch tracking-[0] leading-6">
                  {card.description}
                </p>
              </div>
            </div>
